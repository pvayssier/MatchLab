...
final class MatchesViewController: UIViewController {
    ...
    private var profiles: [Profile]! {
        didSet {
            if profiles.isEmpty {
                view.addSubview(viewEmptyMatches)
            } else {
                viewEmptyMatches.removeFromSuperview()
            }
    ...
    // MARK: - View empty matches

    private lazy var viewEmptyMatches = {
    ...
    func collectionView(_ collectionView: UICollectionView,
        ...
        return UIContextMenuConfiguration(actionProvider: { _ in
            return UIMenu(children: [
                UIAction(title: "Mark as Favorite") { _ in /* Implement the action. */ },
                UIAction(title: "Delete", attributes: .destructive) { [ weak self ] _ in
                    guard let self else { return }
                    let profileName = self.profiles[indexPaths.first!.row].name
